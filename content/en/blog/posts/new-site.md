---
title: "🛠 Site Formations"
date: 2024-02-29
publishDate: 2024-02-09T11:00:00Z
description: >
  Whether bootstrapping a startup, or building a open source project, having a landing site with a real domain name is critical to fostering adoption. In this post, we will look at how to host a static site on Github Pages and automate the deployment process using Github Actions.
---
...
<img src="/images/blogs/spa-hosting/overview.png" alt="Site Formations" style="width: 100%; display: block; margin-left: auto; margin-right: auto;">

This is especially true for a startup like [CodeBlind][codeblind]. The site needs to be secure, fast, and easy to maintain.

Additionally, until we have funding, we need to keep costs low; or free if possible without sacrificing performance. This means we need to use free services as much as possible with high performance characteristics. Luckily for us, [ithub pages][github-pages] is just that!
...
When building SPA, it is possible to deploy two separate sites on github pages. One for the landing site and documentation, and another for the application. This is a great way to provide SEO support for the landing site and documentation, while keeping the application separate. This means doing this exercise twice, and tying a login button on the first to the second site. 

If you want to keep your code private I recommend two things:
1. Use a private repository with a subscription which provides pages customized domains. This will keep your code private. You can still use github pages, but you will have to pay for the service.
2. Obfuscation your code, and strip map files. This not only makes your code smaller to server (a great idea), but also makes it harder (not impossible) to reverse engineer.
