...
(Optional) During cluster creation, to set a specific available version in the release channel, use the `--cluster-version=[VERSION]` flag, e.g. `--cluster-version={{% gke-example-cluster-version %}}`. Be sure to choose a [version supported by Code Blind]({{< relref "../../Installation/#usage-requirements" >}}). (If you rely on release channels, the latest Code Blind release [should be supported]({{< relref "../../Installation/#agones-and-kubernetes-supported-versions" >}}) by the default versions of all channels.)
...
*  **[Scheduling strategy]({{<ref "/docs/Advanced/scheduling-and-autoscaling#fleet-scheduling">}}):**
   `Packed` is supported, which is the Code Blind default. `Distributed` is not
   supported.
*  **[Host port policy]({{<ref "/docs/reference/agones_crd_api_reference#agones.dev/v1.GameServerPort">}}):** `Dynamic` is supported, which is the Code Blind default.
...
*  **[Pod disruption policy]({{<ref "/docs/Advanced/controlling-disruption#eviction-api">}}):**
   `eviction.safe: Never` is supported, which is the Code Blind
